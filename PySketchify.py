...
class ProcessingSession:
    def __init__(self,input_path:Path,temp_dir:Optional[Path]=None,output_path:Optional[Path]=None):
        ...
        self.work_root=self.temp_dir/(input_path.stem+"_pysketchify")
...
def run_video(info:VideoInfo,session:ProcessingSession,settings,progress_callback=None):
    ...
    return run_streaming_pipeline(session.input_path,session.output_path,info.width,info.height,info.fps,info.frame_count,processor=processor,queue_frames=q,threads=1,progress_callback=callback,stop_event=session.stop_event)

class PySketchifyApp:
    def __init__(self,root:tk.Tk):
        self.root=root; self.root.title(f"{APP_NAME} {VERSION}"); self.root.geometry("1080x800")
        self.session=None; self.info=None; self.completed_frames=0; self.processing_frames=0; self.waiting_frames=0
        self.input_var=tk.StringVar(); self.temp_var=tk.StringVar(value=str(default_temp_dir())); self.output_var=tk.StringVar()
        self.status_var=tk.StringVar(value="入力動画を選択してください。"); self.progress_var=tk.DoubleVar(value=0); self.stats_var=tk.StringVar(value="処理済み：0 枚  処理中：0 枚  処理待ち：0 枚")
        self.resource_var=tk.StringVar(); self.info_var=tk.StringVar(value="未選択"); self.pen_width_var=tk.IntVar(value=3); self.pen_type_var=tk.StringVar(value="●")
    ...
    def _build(self):
        ...
        ttk.Label(main,text=APP_NAME,font=("Segoe UI",20,"bold")).pack(anchor="w"); ttk.Label(main,text="動画 → 内部自動お絵描き → 手描き風動画",font=("Segoe UI",10)).pack(anchor="w",pady=(0,12))
        ...
        ttk.Progressbar(main,variable=self.progress_var,maximum=100).pack(fill="x",pady=(10,4)); ttk.Label(main,textvariable=self.stats_var).pack(anchor="w"); ttk.Label(main,textvariable=self.status_var).pack(anchor="w",pady=(4,8))
        buttons=ttk.Frame(main); buttons.pack(fill="x"); self.start_button=ttk.Button(buttons,text="処理開始",command=self.start); self.start_button.pack(side="left"); self.stop_button=ttk.Button(buttons,text="停止",command=self.stop,state="disabled"); self.stop_button.pack(side="left",padx=8)
    def open_pen_width_dialog(self):
        ...
        value=tk.IntVar(value=self.pen_width_var.get()); ttk.Label(frame,text="ペンの太さ(px)").pack(anchor="w"); scale=ttk.Scale(frame,from_=1,to=64,orient="horizontal",command=lambda v:value.set(round(float(v)))); scale.set(value.get()); scale.pack(fill="x",pady=8)
        value_label=ttk.Label(frame,text="3 px"); value_label.pack(); canvas=tk.Canvas(frame,width=300,height=90,highlightthickness=1); canvas.pack(pady=10)
        def refresh(*_):
    ...
    def analyze_input(self):
        try:
            self.info=probe_video(Path(self.input_var.get())); resources=get_resource_snapshot(); temp=Path(self.temp_var.get()) if self.temp_var.get() else default_temp_dir(); temp.mkdir(parents=True,exist_ok=True); free=shutil.disk_usage(temp).free
            self.info_var.set(f"{self.info.width} × {self.info.height} | {self.info.fps:.6g} FPS | {self.info.frame_count:,} frame | audio {self.info.audio_streams} | subtitle {self.info.subtitle_streams}\n内部自動お絵描き: ON | 一時先空き: {format_bytes(free)}")
            ...
            self.status_var.set("解析完了。ペン設定を確認して処理開始できます。")
    ...
    def start(self):
        ...
        temp=Path(self.temp_var.get()).expanduser().resolve() if self.temp_var.get() else default_temp_dir(); self.session=ProcessingSession(Path(self.input_var.get()),temp_dir=temp,output_path=output)
        self.start_button.config(state="disabled"); self.stop_button.config(state="normal"); self.progress_var.set(0); self.completed_frames=0; self.processing_frames=0; self.waiting_frames=self.info.frame_count; self._update_live_counts(); threading.Thread(target=self._worker,args=(settings,),daemon=True).start()
    def _worker(self,settings):
        ...
        try:
            ...
            run_video(self.info,self.session,settings,progress)

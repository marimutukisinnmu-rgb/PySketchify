...
def require_ffmpeg() -> tuple[str, str]:
    ...
    if not ffmpeg or not ffprobe:
        raise RuntimeError("FFmpeg / FFprobe が見つかりません。PATHに ffmpeg と ffprobe を追加してください。")
...
def get_resource_snapshot() -> ResourceSnapshot:
    ...
    total, available = get_ram_snapshot()
    return ResourceSnapshot(total, available, None, None, None, None, ())

def choose_chunk_frame_count(info: VideoInfo, target_gb: float = DEFAULT_CHUNK_TARGET_GB) -> int:
    per_frame = max(1, estimate_frame_bytes(info.width, info.height))
    target = max(64 * 1024**2, int(target_gb * 1024**3))
...
def build_chunk_plan(info: VideoInfo, target_gb: float = DEFAULT_CHUNK_TARGET_GB) -> list[ChunkPlan]:
    ...
    while start <= info.frame_count:
        ...
        count = end - start + 1
        plan.append(ChunkPlan(index, start, end, count, estimate_chunk_bytes(info.width, info.height, count), f"frame-{start:08d}-{end:08d}.mkv"))
        start = end + 1
        index += 1
    return plan
...
class ProcessingSession:
    ...
    def prepare(self):
        ...
        required = add_safety_margin(sum(c.estimated_bytes for c in plan))
        return info, plan, required

    def run_extraction(self, info, plan, progress_callback=None):
        completed = 0
        for chunk in plan:
...
def run_lowres_streaming(info: VideoInfo, session: ProcessingSession, progress_callback=None):
    if run_streaming_pipeline is None:
        raise RuntimeError("streaming_pipeline.py を読み込めません。")
    resources = get_resource_snapshot()
    queue_frames = choose_queue_frames(info.width, info.height, resources.ram_available) if choose_queue_frames else 4
    print(f"[STREAM] 1080p以下のためストリーミング処理を使用")
    print(f"[STREAM] RAM空き: {format_bytes(resources.ram_available) if resources.ram_available else '不明'} | キュー: {queue_frames} frame")
    ...
    def callback(stats):
        ...
        if now - last_report[0] >= 0.25 or (info.frame_count and stats.frames >= info.frame_count):
            last_report[0] = now
            speed = stats.frame_rate
            print(f"処理中... | {stats.frames:,}/{info.frame_count:,} frame | {speed:.1f} frame/s")
    stats = run_streaming_pipeline(session.input_path, session.output_path, info.width, info.height, info.fps, info.frame_count, queue_frames=queue_frames, threads=1, progress_callback=callback)
    print(f"[STREAM] 完了: {stats.frames:,} frame | 平均 {stats.frame_rate:.1f} frame/s | {stats.elapsed:.2f} 秒")
    return stats
...
def console_run(path: Path, temp_dir: Optional[Path] = None, output_path: Optional[Path] = None):
    ...
    print(f"一時先空き容量 : {format_bytes(free)}")
    if free < required:
        raise RuntimeError(f"ディスク容量不足です。必要推定 {format_bytes(required)} / 空き {format_bytes(free)}")
    print("\nチャンク展開を開始します。")
    started = time.perf_counter()
    completed = 0
    for chunk in plan:
        ...
        if output.exists() and output.stat().st_size > 0:
            continue
        extract_chunk(session.input_path, output, chunk.start_frame, chunk.end_frame)
        completed += chunk.frame_count
        elapsed = max(0.001, time.perf_counter() - started)
        print(f"[{chunk.index}/{len(plan)}] {output.name} | {completed:,}/{info.frame_count:,} frame | {completed/elapsed:.1f} frame/s")
    try:
        print(f"完了: {completed:,}/{info.frame_count:,} frame")
    finally:
        session.finish_cleanup()
...
class PySketchifyApp:
    ...
    def _build(self):
        ...
        paths = ttk.LabelFrame(main, text="入出力", padding=10); paths.pack(fill="x")
        ...
        ttk.Label(paths, text="入力なしの場合は (pyがある場所)/tmp。処理終了後、この自動生成tmpは削除します。").pack(anchor="w", padx=(22, 0), pady=(0, 4))
        self._path_row(paths, "出力先", self.output_var, self.choose_output, "保存先")
        info_frame = ttk.LabelFrame(main, text="動画情報 / リソース", padding=10); info_frame.pack(fill="x", pady=12)
        ...
        preview = ttk.LabelFrame(main, text="処理中 / プレビュー", padding=10); preview.pack(fill="both", expand=True)
        ...
        self.start_button = ttk.Button(buttons, text="処理開始", command=self.start); self.start_button.pack(side="left")
        self.stop_button = ttk.Button(buttons, text="停止", command=self.stop, state="disabled"); self.stop_button.pack(side="left", padx=8)
    ...
    def choose_input(self):
        ...
        if path:
            selected = Path(path).resolve(); self.input_var.set(str(selected)); self.output_var.set(str(default_output_path(selected))); self.analyze_input()
    ...
    def choose_output(self):
        ...
        path = filedialog.asksaveasfilename(title="出力先を選択", initialdir=str(current.parent) if current else str(Path.home()), initialfile=current.name if current else "output_pysketchify.mp4", defaultextension=".mp4", filetypes=[("MP4 video", "*.mp4"), ("All files", "*.*")])
        if path: self.output_var.set(str(Path(path).resolve()))

    def analyze_input(self):
        try:
            self.info = probe_video(Path(self.input_var.get()))
            self.plan = build_chunk_plan(self.info)
            resources = get_resource_snapshot()
            if self.info.height <= MAX_STREAMING_HEIGHT:
                mode = "1080p以下 / ストリーミング"
                storage = "全フレーム展開なし"
            else:
                mode = "チャンク / タイル"
                storage = format_bytes(add_safety_margin(sum(c.estimated_bytes for c in self.plan)))
            temp = Path(self.temp_var.get()) if self.temp_var.get() else default_temp_dir(); temp.mkdir(parents=True, exist_ok=True)
            free = shutil.disk_usage(temp).free
            self.info_var.set(f"{self.info.width} × {self.info.height} | {self.info.fps:.6g} FPS | {self.info.frame_count:,} frame | audio {self.info.audio_streams} | subtitle {self.info.subtitle_streams}\nモード: {mode} | 推定必要容量: {storage} | 一時先空き: {format_bytes(free)}")
            self.resource_var.set(f"GPU: {', '.join(resources.gpu_names) if resources.gpu_names else '検出情報なし'} | RAM空き: {format_bytes(resources.ram_available) if resources.ram_available is not None else '不明'} | 専用VRAM総量: {format_bytes(resources.vram_dedicated_total) if resources.vram_dedicated_total is not None else '不明'}")
            self.status_var.set("解析完了。")
        except Exception as exc:
            self.info = None; self.plan = []; self.status_var.set(f"解析エラー: {exc}"); messagebox.showerror(APP_NAME, str(exc))
    ...
    def start(self):
        ...
        self.progress_var.set(0); self.completed_frames = 0; self.processing_frames = 0; self.waiting_frames = self.info.frame_count; self._update_live_counts()
    ...
    def _worker(self):
        ...
        try:
            if self.info.height <= MAX_STREAMING_HEIGHT:
                def progress(stats):
                    self.completed_frames = stats.frames
                    self.processing_frames = 1 if stats.frames < self.info.frame_count else 0
                    self.waiting_frames = max(0, self.info.frame_count - stats.frames - self.processing_frames)
                    percent = stats.frames / self.info.frame_count * 100 if self.info.frame_count else 0
                    self.root.after(0, self._update_stream_progress, percent, stats.frame_rate)
                run_lowres_streaming(self.info, self.session, progress)
                self.completed_frames = self.info.frame_count
                self.processing_frames = 0
                self.waiting_frames = 0
                self.root.after(0, self._update_progress, 100, 0, self.session.output_path.name)
            else:
                started = time.perf_counter()
                for chunk in self.plan:
                    ...
                    self.completed_frames += chunk.frame_count; self.processing_frames = 0
                    speed = self.completed_frames / max(0.001, time.perf_counter() - started)
                    self.root.after(0, self._update_progress, self.completed_frames / self.info.frame_count * 100, speed, output.name)
                self.waiting_frames = max(0, self.info.frame_count - self.completed_frames)
                self.root.after(0, self._update_live_counts)
        except Exception as exc:
            self.root.after(0, self._worker_error, str(exc)); return
        finally:
            if self.session and self.info.height <= MAX_STREAMING_HEIGHT and not self.session.stop_requested:
                self.session.finish_cleanup()
        self.root.after(0, self._worker_done)

    def _update_live_counts(self):
        self.stats_var.set(f"処理済み：{self.completed_frames:,} 枚  処理中：{self.processing_frames:,} 枚  処理待ち：{self.waiting_frames:,} 枚")

    def _update_stream_progress(self, percent, speed):
        self.progress_var.set(percent); self._update_live_counts(); self.status_var.set(f"ストリーミング処理中... | {speed:.1f} frame/s")

    def _update_progress(self, percent, speed, filename):
        self.progress_var.set(percent); self._update_live_counts(); self.status_var.set(f"処理中... | {speed:.1f} frame/s | {filename}")

    def _worker_done(self):
        stopped = self.session.stop_requested if self.session else False
        self.status_var.set("停止しました。" if stopped else f"完了: {self.session.output_path if self.session else ''}")
        self.start_button.config(state="normal"); self.stop_button.config(state="disabled")

    def _worker_error(self, message):
        self.status_var.set(f"エラー: {message}"); self.start_button.config(state="normal"); self.stop_button.config(state="disabled"); messagebox.showerror(APP_NAME, message)

    def stop(self):
        if self.session: self.session.stop(); self.status_var.set("停止要求を送信しました。")
...
def main():
    if len(sys.argv) > 1:
        ...
        console_run(input_path, temp_dir=temp_dir, output_path=output_path); return
    if tk is None:

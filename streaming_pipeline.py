...
def choose_queue_frames(width: int, height: int, ram_available: Optional[int]) -> int:
    ...
    if ram_available is None: return DEFAULT_QUEUE_FRAMES
...
def run_streaming_pipeline(input_path: Path, output_path: Path, width: int, height: int, fps: float,
                           frame_count: int = 0, processor: FrameProcessor = passthrough_processor,
                           queue_frames: Optional[int] = None, threads: int = 1,
    ...
    if height > MAX_STREAMING_HEIGHT: raise ValueError("1080pを超える動画はチャンク/タイル処理を使用してください。")
    ...
    stop = stop_event or threading.Event(); stats = StreamingStats(started=time.perf_counter())
    errors: list[BaseException] = []; errors_lock = threading.Lock()
